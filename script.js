            ...
            plugins: {
                ...
                datalabels: {
                    display: (ctx) => {
                        ...
                        return Number.isFinite(v) && v >= 2.9; // filter < 2.9%
                    },
                    color: '#111827',
                    formatter: (v, ctx) => {
...
function createSectorCallouts(chartData, colors) {
    ...
    // Filter items that should have callouts - responsive based on screen size
    const weightThreshold = window.innerWidth <= 768 ? 4.0 : 2.95; // Higher threshold for mobile to reduce clutter
    ...
    // Responsive radius calculations based on screen size
    const isMobile = window.innerWidth <= 768;
    const baseRadius = Math.min(chartCenterX, chartCenterY) * (isMobile ? 0.55 : 0.6); // Slightly larger radius on mobile for stability
    const baseCalloutRadius = Math.min(chartCenterX, chartCenterY) * (isMobile ? 1.25 : 1.3); // Moderate push out on mobile
    ...
    adjustedPositions.forEach((position, index) => {
        ...
        } else if (item.symbol === 'Communication Services') {
            displayText = 'Internet';
        } else if (item.symbol === 'Semiconductors') {
            displayText = 'Semis';
        }
...
function adjustCalloutPositions(positions, chartCenterX, chartCenterY, baseRadius) {
    ...
    // Responsive callout radius based on screen size
    const isMobile = window.innerWidth <= 768;
    const calloutRadius = baseRadius * (isMobile ? 1.6 : 3.5); // Balanced mobile distance
...
function createAllocationCallouts(chartData, colors) {
    ...
    const baseCalloutRadius = Math.min(chartCenterX, chartCenterY) * (isMobile ? 1.25 : 1.3);
