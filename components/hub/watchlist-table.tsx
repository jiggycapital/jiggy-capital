...
export function WatchlistTable({ watchlistData, logos }: WatchlistTableProps) {
  ...
  const renderChangeCell = (value: number) => (
    <TableCell className={`text-right font-mono text-xs font-bold ${value >= 0 ? 'text-jiggy-neon' : 'text-rose-400'}`}>
        ...
        <Table>
          <TableHeader className="bg-jiggy-surface-2 border-b border-jiggy-tan/50">
            <TableRow className="border-none hover:bg-transparent">
              <TableHead className="w-[200px] text-slate-400 font-bold py-4 sticky left-0 bg-jiggy-surface-2 z-10">Company</TableHead>
          ...
          <TableBody>
            {tableData.map((row, idx) => (
              <TableRow key={`${row.ticker}-${idx}`} className="border-b border-jiggy-tan/30 hover:bg-jiggy-tan/10 transition-colors">
