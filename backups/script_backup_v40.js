...
function updateConsolidatedChart(view = 'company') {
    ...
    // Create logo callouts for company view
    if (view === 'company') {
        setTimeout(() => {
            createChartCallouts(chartData, customColors, false);
...
function createChartCallouts(chartData, colors, isSectorView = false) {
    const selector = isSectorView ? '#sector-allocation .allocation-chart' : '.allocation-chart';
    ...
    const baseCalloutRadius = Math.min(chartCenterX, chartCenterY) * (isMobile ? 1.05 : 1.08);
    ...
    adjustedPositions.forEach((position, index) => {
        ...
        if (isSectorView) {
            ...
            callout.textContent = `${label} ${percent}`;
        } else {
            ...
            // Only show percentage, no ticker symbol
            callout.appendChild(document.createTextNode(percent));
        }
...
function adjustCalloutPositions(positions, chartCenterX, chartCenterY, baseRadius) {
    ...
    const isMobile = window.innerWidth <= 768;
    const calloutRadius = Math.min(chartCenterX, chartCenterY) * (isMobile ? 1.05 : 1.08); // Use same calculation as baseCalloutRadius
