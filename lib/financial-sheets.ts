...
export function parseFinancialSheet(rows: string[][], companyName: string): CompanyFinancialData {
  ...
  // Try to find a ticker in the sheet (usually in the first row or under a "Ticker" / "Symbol" metric)
  let ticker: string | undefined;
  for (let i = 1; i < Math.min(10, rows.length); i++) {
    const row = rows[i];
    const metricNameLower = (row[0] || '').trim().replace(/^"|"$/g, '').toLowerCase();

    // Look for rows that might contain the ticker/symbol
    if (metricNameLower === 'ticker' || metricNameLower === 'symbol') {
      // Find the first column with actual ticker data (usually Q1 column)
      for (let j = 1; j < row.length; j++) {
        const val = (row[j] || '').trim().replace(/^"|"$/g, '');
        if (val && val !== '-' && val !== '#N/A') {
          ticker = val;
          break;
        }
